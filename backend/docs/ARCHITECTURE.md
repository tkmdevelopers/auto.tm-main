...
This document describes the current NestJS backend architecture for Alpha Motors (auto.tm), focusing on authentication, OTP handling, WebSocket communication, and database migrations. It identifies pain points and serves as a reference for the OTP-only refactor.
