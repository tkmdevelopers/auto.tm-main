...
void main() {
  ...
  PostUploadSnapshot createTestSnapshot({
  ...
  group('UploadPersistenceService - Image Hydration', () {
    ...
    test('hydrateImages() handles empty photo list', () {
      final snap = createTestSnapshot(photoBase64: []);
    ...
    test('hydrateImages() handles large base64 payloads', () {
      ...
      final snap = createTestSnapshot(photoBase64: [base64Encode(largeBytes)]);
      ...
      expect(
        images[0][images[0].length - 1],
        largeBytes[largeBytes.length - 1],
      );
    });
  });
  ...
  group('UploadPersistenceService - Video Validation', () {
    test('hasValidVideoFile() returns false when no video in snapshot', () {
      final snap = createTestSnapshot(hasVideo: false);

      expect(service.hasValidVideoFile(snap), false);
    });

    test('hasValidVideoFile() returns false when video file is null', () {
      final snap = createTestSnapshot(hasVideo: true, videoFile: null);

      expect(service.hasValidVideoFile(snap), false);
    });

    test(
      'hasValidVideoFile() returns false when video file does not exist',
      () {
        final snap = createTestSnapshot(
          hasVideo: true,
    ...
    test(
      'hasValidVideoFile() checks compressed video when usedCompressedVideo is true',
      () {
        final snap = createTestSnapshot(
          hasVideo: true,
  ...
  group('UploadPersistenceService - Crash Recovery Logic', () {
    ...
    test(
      'clearTask() does not throw when called once (platform channel test skipped)',
      () {
    ...
    test(
      'persistTask() handles task with minimal data (platform test skipped)',
      () {
    ...
    test(
      'persistTask() handles task with all progress fields (platform test skipped)',
      () {
        // Note: persistTask() calls GetStorage which requires platform channels
        // This is tested in integration tests instead
        expect(service, isNotNull);
    ...
    test(
      'persistTask() handles task in failed state (platform test skipped)',
      () {
        // Note: persistTask() calls GetStorage which requires platform channels
        // This is tested in integration tests instead
        expect(service, isNotNull);
    ...
    test(
      'persistTask() handles task in completed state (platform test skipped)',
      () {
        // Note: persistTask() calls GetStorage which requires platform channels
        // This is tested in integration tests instead
        expect(service, isNotNull);
    ...
    test(
      'persistTask() handles task in cancelled state (platform test skipped)',
      () {
        // Note: persistTask() calls GetStorage which requires platform channels
        // This is tested in integration tests instead
        expect(service, isNotNull);
  ...
  group('UploadPersistenceService - Snapshot Edge Cases', () {
    test(
      'hydrateImages() handles snapshot with mix of valid/invalid base64',
      () {
    ...
    test(
      'hasValidVideoFile() handles snapshot with both video files present',
      () {
        final snap = createTestSnapshot(
          hasVideo: true,
