...
class FilterScreen extends StatelessWidget {
            ...
            children: [
                      ...
                      children: [
                        ...
                        _buildSelectorTile(
                          ...
                          controller.selectedColor.value.isEmpty
                              ? 'common_any'.tr
                              : controller.selectedColor.value.tr,
                            ...
                            return GestureDetector(
                              ...
                              child: Obx(() {
                                ...
                                return Container(
                                  ...
                                  decoration: BoxDecoration(
                                    border: Border.all(
                                      color: isSelected ? theme.colorScheme.onSurface : AppColors.textTertiaryColor,
                                      width: 0.3,
                                    ),
              ...
              Positioned(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: theme.colorScheme.surface,
                    border: Border(top: BorderSide(color: theme.colorScheme.outlineVariant.withOpacity(.25), width: 0.7)),
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Text(title, style: TextStyle(fontWeight: FontWeight.w500, color: theme.colorScheme.onSurface, fontSize: 16)),
  ...
  Widget _buildSelectorTile(String title, String value, void Function() onTap, BuildContext context) {
    final theme = Theme.of(context);
    return Column(
      children: [
        ListTile(
          contentPadding: EdgeInsets.zero,
          title: Text(title, style: TextStyle(fontWeight: FontWeight.w500, color: theme.colorScheme.onSurface, fontSize: 16)),
          trailing: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(value, style: const TextStyle(color: AppColors.textTertiaryColor)),
              const Icon(Icons.chevron_right, color: AppColors.textTertiaryColor),
            ],
          ),
          onTap: onTap,
        ),
  ...
  Widget _buildCheckbox(String title, RxBool value, Color color) {
    return Obx(
      () => CheckboxListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(title, style: TextStyle(fontWeight: FontWeight.w400, color: color.withOpacity(0.9), fontSize: 14)),
        value: value.value,
  ...
  void showOptionsBottomSheet(BuildContext context, String title, List<String> options, RxString selectedValue) {
    final theme = Theme.of(context);
    void _safeSheetPop() {
      ...
      SafeArea(
        ...
        child: Material(
          ...
          child: Column(
            ...
            children: [
              const SizedBox(height: 8),
              Container(width: 42, height: 5, decoration: BoxDecoration(color: AppColors.textTertiaryColor.withOpacity(0.5), borderRadius: BorderRadius.circular(3))),
              const SizedBox(height: 8),
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    IconButton(icon: Icon(Icons.close, color: AppColors.textTertiaryColor, size: 20), onPressed: _safeSheetPop),
                  ],
                ),
              ),
              Flexible(
                child: ListView.separated(
                  ...
                  separatorBuilder: (_, __) => Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Divider(height: 1, thickness: 0.5, color: AppColors.textTertiaryColor.withOpacity(0.6))),
                  itemBuilder: (context, index) {
                    final opt = options[index];
                    return Obx(() {
                      final bool selected = selectedValue.value == opt;
                      return InkWell(
                        onTap: () { selectedValue.value = opt; _safeSheetPop(); },
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                          color: selected ? theme.colorScheme.onSurface.withOpacity(0.05) : Colors.transparent,
                          child: Row(
                            children: [
                              Expanded(child: Text(opt, style: TextStyle(fontSize: 16, fontWeight: selected ? FontWeight.w600 : FontWeight.w400, color: theme.colorScheme.onSurface))),
                              Radio<String>(value: opt, groupValue: selectedValue.value, onChanged: (val) { if (val != null) { selectedValue.value = val; _safeSheetPop(); } }, activeColor: theme.colorScheme.onSurface, visualDensity: VisualDensity.compact),
                            ],
                          ),
                        ),
                      );
  ...
  List<Widget> _buildActiveFilterChips(FilterController c, ThemeData theme) {
    ...
    if (c.selectedBrandName.value.isNotEmpty) {
      add('Brand: ${c.selectedBrandName.value}', () {
        c.selectedBrandName.value=''; c.selectedBrandUuid.value=''; c.selectedModelName.value=''; c.selectedModelUuid.value='';
      });
    }
    if (c.selectedModelName.value.isNotEmpty) {
      add('Model: ${c.selectedModelName.value}', () { c.selectedModelName.value=''; c.selectedModelUuid.value=''; });
    }
    if (c.selectedCategoryName.value.isNotEmpty) {
      add('Category: ${c.selectedCategoryName.value}', () { c.selectedCategoryName.value=''; c.selectedCategoryUuid.value=''; });
    }
    ...
    if (c.exchange.value) { add('Exchange', () { c.exchange.value=false; }); }
    if (c.credit.value) { add('Credit', () { c.credit.value=false; }); }
    if (c.selectedColor.value.isNotEmpty) { add('Color: ${c.selectedColor.value}', () { c.selectedColor.value=''; }); }
    if (c.premium.isNotEmpty) { add('Premium: ${c.premium.length}', () { c.premium.clear(); }); }
    if (c.condition.value.isNotEmpty && c.condition.value != 'All') { add('Cond: ${c.condition.value}', () { c.condition.value='All'; }); }
    return chips;
  }
  ...
  Widget _buildBrandModelSummary(ThemeData theme, FilterController controller) {
    return Obx(() => Column(
      ...
      children: [
        if (controller.selectedBrandName.value.isNotEmpty)
          _inlineKeyValue(theme, 'Brand'.tr, controller.selectedBrandName.value, onRemove: () {
            controller.selectedBrandName.value=''; controller.selectedBrandUuid.value=''; controller.selectedModelName.value=''; controller.selectedModelUuid.value='';
          }),
        if (controller.selectedModelName.value.isNotEmpty)
          _inlineKeyValue(theme, 'Model'.tr, controller.selectedModelName.value, onRemove: () { controller.selectedModelName.value=''; controller.selectedModelUuid.value=''; }),
      ],
    ));
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          if (onRemove != null)
            IconButton(onPressed: onRemove, icon: Icon(Icons.close, size: 18, color: AppColors.textTertiaryColor), visualDensity: VisualDensity.compact, padding: EdgeInsets.zero),
        ],
      ),
    );
  }

  void _showCategoryBottomSheet(BuildContext context, FilterController controller) {
    final theme = Theme.of(context);
              ...
              child: Obx(() {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSelected = controller.selectedCategoryUuid.value == category.uuid;
                    return ListTile(
                      title: Text(
                        category.nameEn,
                        style: TextStyle(
                      ...
                      trailing: isSelected
                          ? Icon(Icons.check_circle_rounded, color: theme.colorScheme.onSurface)
                          : null,
                      onTap: () {
                        controller.selectCategory(category.uuid, category.nameEn);
                        Get.back();
                      },
                    );
                  },
  ...
  Widget _buildNumericField({required String label, required TextEditingController controller, required ThemeData theme}) {
    return Column(
      children: [
        Row(
          children: [
            Expanded(flex: 2, child: Text(label, style: TextStyle(fontWeight: FontWeight.w500, color: theme.colorScheme.onSurface, fontSize: 16))),
            Expanded(flex: 1, child: Padding(padding: const EdgeInsets.symmetric(vertical: 10.0), child: STextField(controller: controller, hintText: '0', type: const TextInputType.numberWithOptions()))),
          ],
        ),
        const Divider(height: 0.5, color: AppColors.textTertiaryColor),
      ],
...
class _YearRangeSlider extends StatelessWidget {
          ...
          Row(
            children: [
              ...
              if (rv.start != lower.toDouble() || rv.end != upper.toDouble())
                TextButton(onPressed: () { controller.yearRange.value = RangeValues(lower.toDouble(), upper.toDouble()); }, child: Text('Reset'.tr, style: TextStyle(fontSize: 12))),
            ],
          ),
          RangeSlider(
            min: lower.toDouble(), max: upper.toDouble(), divisions: (upper - lower),
            labels: RangeLabels(rv.start.round().toString(), rv.end.round().toString()),
            values: rv,
            onChanged: (vals) { controller.yearRange.value = vals; },
            onChangeEnd: (_) { controller.minYear.value = ''; controller.maxYear.value = ''; },
          ),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [_yearPill(theme, rv.start.round()), _yearPill(theme, rv.end.round())]),
  ...
  Widget _yearPill(ThemeData theme, int year) {
    return Container(
      ...
      decoration: BoxDecoration(color: theme.colorScheme.surfaceVariant.withOpacity(.4), borderRadius: BorderRadius.circular(20), border: Border.all(color: theme.colorScheme.outlineVariant.withOpacity(.4), width: 0.6)),
      child: Text(year.toString(), style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: theme.colorScheme.onSurface)),
    );
  }
}
...
class _PriceRangeSlider extends StatelessWidget {
          ...
          Row(
            children: [
              ...
              if (rv.start != lower || rv.end != upper)
                TextButton(onPressed: () { controller.priceRange.value = RangeValues(lower, upper); controller.minPrice.value = null; controller.maxPrice.value = null; }, child: Text('Reset'.tr, style: TextStyle(fontSize: 12))),
            ],
          ),
          RangeSlider(
            min: lower, max: upper, divisions: 100,
            labels: RangeLabels(_fmt(rv.start), _fmt(rv.end)),
            values: rv,
            onChanged: (vals) { controller.priceRange.value = vals; },
            onChangeEnd: (_) { controller.minPrice.value = null; controller.maxPrice.value = null; },
          ),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [_pricePill(theme, rv.start), _pricePill(theme, rv.end)]),
          const SizedBox(height: 4),
          const Divider(height: 0.5, color: AppColors.textTertiaryColor),
        ],
      );
  ...
  Widget _pricePill(ThemeData theme, double value) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
      decoration: BoxDecoration(color: theme.colorScheme.surfaceVariant.withOpacity(.4), borderRadius: BorderRadius.circular(20), border: Border.all(color: theme.colorScheme.outlineVariant.withOpacity(.4), width: 0.6)),
      child: Text(_fmt(value), style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: theme.colorScheme.onSurface)),
    );
  }
}